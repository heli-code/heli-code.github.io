...
public interface IPlaylistService extends IService<Playlist> {
    ...
    // 添加歌单
    Result<String> addPlaylist(PlaylistAddDTO playlistAddDTO);

    // 更新歌单
    Result<String> updatePlaylist(PlaylistUpdateDTO playlistUpdateDTO);

    // 更新歌单封面
    Result<String> updatePlaylistCover(Long playlistId, String coverUrl);

    // 删除歌单
    Result<String> deletePlaylist(Long playlistId);

    // 批量删除歌单
    Result<String> deletePlaylists(List<Long> playlistIds);
