...
public interface IArtistService extends IService<Artist> {
    ...
    // 添加歌手
    Result<String> addArtist(ArtistAddDTO artistAddDTO);

    // 更新歌手
    Result<String> updateArtist(ArtistUpdateDTO artistUpdateDTO);

    // 更新歌手头像
    Result<String> updateArtistAvatar(Long artistId, String avatar);

    // 删除歌手
    Result<String> deleteArtist(Long ArtistId);

    // 批量删除歌手
    Result<String> deleteArtists(List<Long> artistIds);
