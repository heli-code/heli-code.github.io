...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
     ...
     */
    @Override
    public Result<String> updateUserPassword(UserPasswordDTO userPasswordDTO, String token) {
        Map<String, Object> map = ThreadLocalUtil.get();
        Object userIdObj = map.get(JwtClaimsConstant.USER_ID);
        Long userId = TypeConversionUtil.toLong(userIdObj);
        User user = userMapper.selectById(userId);
     ...
     */
    @Override
    public Result<String> resetUserPassword(UserResetPasswordDTO userResetPasswordDTO) {
        // 删除Redis中的验证码
    ...
    @Override
    @CacheEvict(cacheNames = "userCache", allEntries = true)
    public Result<String> addUser(UserAddDTO userAddDTO) {
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
    ...
    @Override
    @CacheEvict(cacheNames = "userCache", allEntries = true)
    public Result<String> updateUserStatus(Long userId, Integer userStatus) {
        // 确保用户状态有效
